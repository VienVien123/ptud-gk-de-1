...
def edit_post(request, id):
    post = get_object_or_404(Post, id=id)
    user_role, created = UserRole.objects.get_or_create(user=request.user)

    if user_role.role not in ['collaborator', 'editor']:
        messages.error(request, "You don't have permission to edit this post.")
        return redirect('posts')
    ...
    return render(request, 'blog/post_form.html', {'form': form, 'id': id})
